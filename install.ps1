# PyPDFSeal Install Script for Windows
# Usage: irm https://raw.githubusercontent.com/CarryDream/PyPDFSeal/main/install.ps1 | iex
#
# Parameters (set before running):
#   $Version = "0.1.3"   # Install specific version
...
function Get-ReleaseVersion {
    ...
    try {
        $release = Invoke-RestMethod -Uri "$GithubApi/latest" -Headers @{
            "User-Agent" = "PyPDFSeal-Installer"
    ...
    try {
        $updaterJson = Invoke-RestMethod -Uri "https://github.com/$Repo/releases/latest/download/latest.json" -TimeoutSec 10
    ...
    Write-Host '  $Version = "0.1.3"; irm https://raw.githubusercontent.com/CarryDream/PyPDFSeal/main/install.ps1 | iex' -ForegroundColor Yellow
    return $false
}

function Get-DownloadUrl {
    # NSIS installer: PyPDFSeal_0.1.3_x64-setup.exe
    $script:DownloadUrl = "https://github.com/$Repo/releases/download/v$($script:ReleaseVersion)/PyPDFSeal_$($script:ReleaseVersion)_x64-setup.exe"
    $script:Filename = "PyPDFSeal_$($script:ReleaseVersion)_x64-setup.exe"
