#!/usr/bin/env bash
# PyPDFSeal Install Script (Linux + macOS)
# Usage: curl -fsSL https://raw.githubusercontent.com/CarryDream/PyPDFSeal/main/install.sh | bash
#
# Environment variables:
#   VERSION     - Install specific version (e.g., "0.1.3"), default: latest
...
REPO="CarryDream/PyPDFSeal"
APP_NAME="PyPDFSeal"
APP_ID="com.pydfseal.app"
...
Supported Platforms:
    - Linux x86_64:  .deb (Debian/Ubuntu), .AppImage (Universal)
    - Linux aarch64: .deb (Debian/Ubuntu), .AppImage (Universal)
    - macOS x86_64:  .dmg
    - macOS arm64:   .dmg
...
get_version() {
    ...
    local response
    if response=$(curl -fsSL -H "User-Agent: PyPDFSeal-Installer" "${GITHUB_API}/latest" 2>/dev/null); then
...
build_download_url() {
    ...
    case "$PLATFORM" in
        linux)
            case "$PKG_EXT" in
                deb)
                    # PyPDFSeal_0.1.3_amd64.deb or _arm64.deb
                    DOWNLOAD_URL="${base_url}/PyPDFSeal_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    FILENAME="PyPDFSeal_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    ;;
                AppImage)
                    # PyPDFSeal_0.1.3_amd64.AppImage or _aarch64.AppImage
                    local appimage_arch
                    ...
                    DOWNLOAD_URL="${base_url}/PyPDFSeal_${RELEASE_VERSION}_${appimage_arch}.AppImage"
                    FILENAME="PyPDFSeal_${RELEASE_VERSION}_${appimage_arch}.AppImage"
                    ;;
                rpm)
                    # Fallback (if rpm is ever added in the future)
                    DOWNLOAD_URL="${base_url}/PyPDFSeal-${RELEASE_VERSION}-1.${RPM_ARCH:-x86_64}.rpm"
                    FILENAME="PyPDFSeal-${RELEASE_VERSION}-1.${RPM_ARCH:-x86_64}.rpm"
                    ;;
            esac
            ;;
        macos)
            # macOS DMG naming (Tauri default)
            # Try universal first, then fall back to architecture-specific
            if [[ "$ARCH_LABEL" == "x86_64" ]]; then
                DOWNLOAD_URL="${base_url}/PyPDFSeal_${RELEASE_VERSION}_x64.dmg"
                FILENAME="PyPDFSeal_${RELEASE_VERSION}_x64.dmg"
            else
                DOWNLOAD_URL="${base_url}/PyPDFSeal_${RELEASE_VERSION}_aarch64.dmg"
                FILENAME="PyPDFSeal_${RELEASE_VERSION}_aarch64.dmg"
            fi
            ;;
    esac
...
install_linux() {
    ...
    case "$PKG_MANAGER" in
        ...
        appimage)
            ...
            run cp "$DOWNLOAD_PATH" "${install_dir}/pypdfseal"

            if [[ ":$PATH:" != *":${install_dir}:"* ]]; then
                warn "Add ${install_dir} to your PATH to run pypdfseal from anywhere"
            ...
            success "${APP_NAME} installed as AppImage to ${install_dir}/pypdfseal"
            ;;
    esac

    if [[ "$PKG_MANAGER" != "appimage" ]]; then
